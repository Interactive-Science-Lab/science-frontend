...
import formHelpers from 'main/defaultComponent/forms/form_helpers'
...
class Page extends React.Component {
    ...
    render() {
        ...
        return <div>
            {field.settings.label ? <div>{formHelpers.printifyName(field.settings.fieldName)}:</div> : ""}



            {field.settings.fieldType === 'string' ? <div>
                {field.settings.titleField ? (action === 'view' ?
                    (settings.get('viewTitle') ? <h2>{field.value}</h2> : <h1>{field.value}</h1>)
